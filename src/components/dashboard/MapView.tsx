...
export function MapView({ trackers }: MapViewProps) {
  ...
  // Mock map with positioned tracker dots on a grid
  ...
  return (
    <div className="glass-card overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-border/50">
        ...
        <button className="glass-card p-1.5 hover:bg-secondary/50 transition-colors">
          <Maximize2 className="w-3.5 h-3.5 text-muted-foreground" />
        </button>
      </div>

      {/* Map Area */}
      <div className="relative h-[400px] bg-[hsla(222,40%,6%,0.9)]">
        {/* Grid lines */}
        ...
        {/* Road-like paths */}
        <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <path d="M 10% 20% Q 30% 40%, 50% 35% T 90% 60%" stroke="hsla(210,40%,96%,0.06)" strokeWidth="2" fill="none" />
          <path d="M 20% 80% Q 40% 50%, 60% 55% T 85% 25%" stroke="hsla(210,40%,96%,0.06)" strokeWidth="2" fill="none" />
          <path d="M 5% 50% L 95% 50%" stroke="hsla(210,40%,96%,0.04)" strokeWidth="1.5" fill="none" strokeDasharray="8,4" />
        </svg>

        {/* Region Labels */}
        <span className="absolute top-[15%] left-[20%] text-[9px] text-muted-foreground/30 font-mono">NORTH DELHI</span>
        <span className="absolute top-[50%] left-[10%] text-[9px] text-muted-foreground/30 font-mono">GURGAON</span>
        <span className="absolute top-[30%] right-[15%] text-[9px] text-muted-foreground/30 font-mono">NOIDA</span>
        <span className="absolute bottom-[20%] left-[40%] text-[9px] text-muted-foreground/30 font-mono">FARIDABAD</span>
        ...
        {selectedTracker && (
          <div
            className="absolute z-20 glass-card p-3 w-52 animate-slide-up"
            style={{
